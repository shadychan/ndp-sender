...
void usage() {
    fprintf(stdout, "Usage: send_rs <target_ipv6> <device>\n");
}
...
static int send_ndp_rs(struct sockaddr_in6 *target, char *dev) {
    int ret = -1, byte_sent, fd;
    struct ifreq ifr_if = {0}, ifr_mac;
    struct ndp_rs_pkt pkt = {0};
    ...
    ret = 0;
    fprintf(stdout, "successfully sent NDP Router Solicitation (RS), sent bytes: %d\n", byte_sent);
err_ret:
    close(fd);
    return ret;
}
...
int main(int argc, char **argv) {
    ...
    if (send_ndp_rs((struct sockaddr_in6 *)&target_addr, argv[2]) < 0) {
